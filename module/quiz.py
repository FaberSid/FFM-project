...
class Quiz(c.Cog):
    ...
    async def q(self, ctx):
        ...
        await ctx.send(discord.Embed(description="Q. {}\n 1. {}\n 2. {}\n 3. {}\n 4. {}".format(quiz_xml[1].text, *quiz_set)))
        ...
        try:
            ...
            await ctx.send(discord.Embed(description='時間切れだ。正解は「{}」だ。'.format(quiz_xml[2].text)))
            return
        ...
        if guess.content.isdigit() and int(guess.content) == answer_num:
            ...
            await ctx.send(discord.Embed(description='残念！正解は「{}」だ。'.format(quiz_xml[2].text)))
